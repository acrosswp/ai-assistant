...
abstract class AbstractOpenAiCompatibleTextGenerationModel extends AbstractApiBasedModel implements
	...
	protected function prepareGenerateTextParams( array $prompt ): array {
		...
		error_log( '[AI Client SDK] Final request params: ' . json_encode( $params, JSON_PRETTY_PRINT ) );
		return $params;
	}
	...
	protected function getMessagePartContentData( MessagePart $part ): ?array {
		...
		if ( $type->isText() ) {
			...
			error_log( '[AI Client SDK] Creating text content data: ' . json_encode( $content_data ) );
			return $content_data;
		}
		if ( $type->isFile() ) {
			$file = $part->getFile();
